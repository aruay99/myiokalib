...
BASE_URL = 'https://stage-api.ioka.kz/v2/customers'

@responses.activate
def test_get_customers():
    responses.add(responses.GET, BASE_URL, json={'data': 'test_data'}, status=200)
    customer = Customer()
    data = customer.get_customers(limit=10, page=1)
...
def test_create_customer():
    ...
    customer = Customer()
    data = customer.create_customer( external_id='external_id', email='email@example.com', phone='1234567890')
...
def test_get_customer_by_id():
    ...
    customer = Customer()
    data = customer.get_customer_by_id( customer_access_token= , customer_id='customer_id')
...
def test_delete_customer_by_id():
    ...
    customer = Customer()
    response = customer.delete_customer_by_id( customer_id='customer_id')
